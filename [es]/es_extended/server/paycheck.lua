ESX.StartPayCheck = function()
	...
	function payCheck()
		...
		for i=1, #xPlayers, 1 do
			...
			local job3    = xPlayer.job3.grade_name
			local salary  = xPlayer.job.grade_salary
			local salary2 = xPlayer.job2.grade_salary
			local salary3 = xPlayer.job3.grade_salary
			...
			if salary3 > 0 then 
				if job3 == 'unemployed3' then -- unemployed
				xPlayer.addAccountMoney('bank', salary3)
					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck3'), _U('received_help3', salary3), 'CHAR_BANK_MAZE', 9)
				elseif Config.EnableSocietyPayouts then -- possibly a society
					TriggerEvent('esx_society:getSociety', xPlayer.job3.name, function (society)
						if society ~= nil then -- verified society
							TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function (account)
								if account.money >= salary3 then -- does the society money to pay its employees?
									xPlayer.addAccountMoney('bank', salary3)
									account.removeMoney(salary3)
	
									TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck3'), _U('received_salary3', salary3), 'CHAR_BANK_MAZE', 9)
								else
									TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
								end
							end)
