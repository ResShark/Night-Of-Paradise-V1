function CreateExtendedPlayer(player, accounts, inventory, job, job2, job3, loadout, name, lastPosition)
	local self = {}
	...
	self.setJob3 = function(job3, grade3)
		...
		if ESX.DoesJob3Exist(job3, grade3) then
			...
			TriggerClientEvent('esx:setJob3', self.source, self.job3)
		else
			print(('es_extended: ignoring setJob3 for %s due to job3 not found!'):format(self.source))
		end
	end
