...
ESX.SavePlayer = function(xPlayer, cb)
	...
	-- Job,Job2,Job3, loadout and position
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET `job` = @job, `job2` = @job2, `job3` = @job3, `job_grade` = @job_grade, `job2_grade` = @job2_grade, `job3_grade` = @job3_grade, `loadout` = @loadout, `position` = @position WHERE identifier = @identifier', {
			['@job']        = xPlayer.job.name,
			['@job2']       = xPlayer.job2.name,
			['@job3']       = xPlayer.job3.name,
			['@job_grade']  = xPlayer.job.grade,
			['@job2_grade']  = xPlayer.job2.grade,
			['@job3_grade']  = xPlayer.job3.grade,
...
ESX.DoesJob3Exist = function(job3, grade3)
