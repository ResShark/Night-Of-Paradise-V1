AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			-- Get job3 name, grade3 and last position
			table.insert(tasks2, function(cb2)

				MySQL.Async.fetchAll('SELECT job3, job3_grade, loadout, position FROM `users` WHERE `identifier` = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
					local job3, grade3 = result[1].job3, tostring(result[1].job3_grade)
					...
					if ESX.DoesJobExist(job3, grade3) then
						...
						if grade3Object.skin_male ~= nil then
							userData.job3.skin_male = json.decode(grade3Object.skin_male)
						end
			
						if grade3Object.skin_female ~= nil then
							userData.job3.skin_female = json.decode(grade3Object.skin_female)
						end
					else
						print(('es_extended: %s had an unknown job3 [job: %s, grade: %s], setting as unemployed3!'):format(player.getIdentifier(), job3, grade3))

						local job3, grade3 = 'unemployed3', '0'
						local job3Object, grade3Object = ESX.Jobs[job3], ESX.Jobs[job3].grades[grade3]
		...
		Async.parallel(tasks, function(results)
			...
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.job2, userData.job3, userData.loadout, userData.playerName, userData.lastPosition)
