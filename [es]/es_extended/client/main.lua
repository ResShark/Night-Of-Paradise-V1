...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	if Config.EnableHud then
		...
		local job3Tpl = '<div>{{job3_label}} - {{grade3_label}}</div>' ---TRIPLEJOB INCLUDED
		
		if xPlayer.job3.grade_label == '' then
		    job3Tpl = '<div>{{job3_label}}</div>'
		end
		
		ESX.UI.HUD.RegisterElement('job3', #xPlayer.accounts, 0, job3Tpl, {
		    job3_label   = '',
			grade3_label = ''
        })
		
		ESX.UI.HUD.UpdateElement('job3', {
	        job3_label   = xPlayer.job3.label,
            grade3_label = xPlayer.job3.grade_label
        })

		local moneyTpl = '<div><img src="img/accounts/cash.png"/>{{money}}</div>' -- modifier

		ESX.UI.HUD.RegisterElement('money', 1, 0, moneyTpl, {   					-- modifier
			money = 0
		})

		ESX.UI.HUD.UpdateElement('money', {   										-- modifier
			money = xPlayer.money
		})
...
AddEventHandler('esx:setJob3', function(job3)
  ESX.UI.HUD.UpdateElement('job3', {
    job3_label   = job3.label,
    grade3_label = job3.grade_label
  })
end)
...
function SetData()
	...
	Citizen.InvokeNative(GetHashKey("ADD_TEXT_ENTRY"), 'FE_THDR_GTAO', '~y~Base By Angel~t~|~b~Identifiacation: ' .. id .. ' ~t~| ~b~Nom RP: ~b~' .. name .. " ~r~Joueurs Connecté: " .. #players .. "/32")
end
