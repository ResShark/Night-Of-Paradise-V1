...
function OpenMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
        ...
        if skin.sex == 0 then
					local clothesSkin = {
            		['tshirt_1'] = 59,  ['tshirt_2'] = 1, --- gilet orange
            		['torso_1']  = 255, ['torso_2']  = 22, -- polo
            		['arms']     = 1, ----------------------- bras
            		['pants_1']  = 59, 	['pants_2']  = 0, --- pantalon
            		['shoes_1']  = 81,  ['shoes_2']  = 0, --- chaussures
            		['helmet_1'] = 2,   ['helmet_2'] = 0, --- casque
            		['bags_1']   = 41,  ['bags_2']   = 0 ---- sac
					}
...
AddEventHandler('esx_jobs:action', function(job2, zone)
	menuIsShowed = true
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job2 ~= nil and PlayerData.job2.name ~= 'unemployed' then
			local zones = nil
			local job2 = nil
			...
			if zones ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					if onDuty or zone.Type == "cloakroom" or PlayerData.job2.name == "reporter" then
						TriggerEvent('esx_jobs:action', job2, zone)
					end
				end
