...
AddEventHandler('salty_crafting:craftItem', function(ingredients)
	...
	if not item then
		TriggerClientEvent('esx:showNotification', _source, "Aucune recette trouver pour c'est ingredients")
	else
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Item fabriquer: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, "Tu n'a pas tous les ingredients")
			end
		end
	end
end)
