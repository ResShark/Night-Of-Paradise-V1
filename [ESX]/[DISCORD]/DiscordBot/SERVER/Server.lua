...
if DiscordWebhookCheat == 'WEBHOOK_LINK_HERE' then
	print('\n\nERROR\n' .. GetCurrentResourceName() .. ': Please add your "Cheat" webhook\n\n')
else
	PerformHttpRequest(DiscordWebhookCheat, function(Error, Content, Head)
		if Content == '{"code": 50027, "message": "Invalid Webhook Token"}' then
			print('\n\nERROR\n' .. GetCurrentResourceName() .. ': "Cheat" webhook non-existing!\n\n')
		end
	end)
end
...
-- System Infos
PerformHttpRequest(DiscordWebhookStart, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**@everyone Le serveur a fini de chargé, vous avez maintenant l\'autorisation de le rejoindre grace à l\'ip suivante : "51.68.62.65"**'}), { ['Content-Type'] = 'application/json' })
