...
function OpenCloakroom()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'alvionconcept_cloakroom',
	{
...
function OpenAlvionActionsMenu()
	local elements = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'alvion_actions', {
		title    = 'Alvion Concept',
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'alvionconcept', function(data, menu)
				menu.close()
		...
		CurrentAction     = 'alvion_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end)
end

function OpenMobileAlvionActionsMenu()
ESX.UI.Menu.CloseAll()
                  ...
                  function(data2, menu2)
                    ...
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                      if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_alvionconcept', _U('alvionconcept'), amount)
                      end
                    end
                  end,
                function(data2, menu2)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_alvionconceptjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_alvionconceptjob:getStockItem', itemName, count)
					Citizen.Wait(1000)
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_alvionconceptjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_alvionconceptjob:putStockItems', itemName, count)
					Citizen.Wait(1000)
					OpenPutStocksMenu()
				end
			end, function(data2, menu2)
...
AddEventHandler('esx_alvionconceptjob:hasEnteredMarker', function(zone)
	if zone == 'AlvionActions' then
		CurrentAction     = 'alvion_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
...
AddEventHandler('esx_alvionconceptjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('phone_alvionconcept'),
		number     = 'alvionconcept',
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'alvionconcept' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_alvionconceptjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_alvionconceptjob:hasExitedMarker', LastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction and not IsDead then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'alvionconcept' then
				if CurrentAction == 'alvion_actions_menu' then
					OpenAlvionActionsMenu()
		...
		if IsControlJustReleased(0,  Keys['F6']) and IsInputDisabled(0) and not IsDead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == 'alvionconcept' then
			OpenMobileAlvionActionsMenu()
		end
	end
end)
 ...
 Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)    
                ...
                DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~r~ Alvion Concept ~d~", 255, 255, 255, 255, 1, 0)
...
RegisterNetEvent('esx_alvionconceptjob:annonce')
AddEventHandler('esx_alvionconceptjob:annonce', function(text)
    texteafiche = text
    affichenews = true
...
RegisterNetEvent('esx_alvionconceptjob:annoncestop')
AddEventHandler('esx_alvionconceptjob:annoncestop', function()
    affichenews = false
    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 30)
        ...
        if (GetOnscreenKeyboardResult()) then
           ...
           TriggerServerEvent('esx_alvionconceptjob:annonce',result)
