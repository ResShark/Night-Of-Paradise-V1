...
function LoadSellPlace()
	Citizen.CreateThread(function()
		...
		local BlipPos = Config.BlipPosition
		...
		AddTextComponentString("Alvion Concept")
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if PlayerData.job ~= nil and PlayerData.job.name == 'alvionconcept'  then
					...
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							...
							ESX.ShowNotification("Vous devez vous asseoir dans un ~g~vehicule")
						end
					end
				end
			end
					...
					if dstCheck <= 2.0 then
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								ESX.ShowNotification("Vous devez vous asseoir dans un ~g~vehicule~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			...
			table.insert(elements, { ["label"] = "Mis en vente", ["value"] = "sell" })
		else
			table.insert(elements, { ["label"] = "Prix fixe - :-", ["value"] = "price" })
		end
	else
		table.insert(elements, { ["label"] = "Acheter " .. price .. " - :-", ["value"] = "buy" })

		if owner then
			table.insert(elements, { ["label"] = "Supprimer le véhicule", ["value"] = "remove" })
		end
	end
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("Vous éteignez le ~g~vehicule~s~ à vendre - " .. price .. " :-")
					menu.close()
				else
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					--ESX.ShowNotification("Vous ~g~acheter~s~ le véhicule pour " .. price .. " :-")
					menu.close()
				--else
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("Tu à ~g~suprimer~s~ le véhicule")
					menu.close()
				end
