								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if alertPolice == true then
										if SpeedKM > alertSpeed then
											...
											TriggerServerEvent('esx_phone:send', 'police', " Quelqu'un est passée devant la camera,a cette vitesse " .. alertSpeed.. ' KMH', true, {x =x, y =y, z =z})
										end
									end
									-- ALERT POLICE (END)								
									...
									TriggerEvent("pNotify:SendNotification", {text = "Tu as été pris par le radar dans la zone 60 ! ta vitesse: " .. math.floor(SpeedKM) .. " KM/H", type = "error", timeout = 5000, layout = "centerLeft"})
									...
									hasBeenCaught = true
									Citizen.Wait(5000) -- C'est là pour s'assurer que le joueur ne sera pas condamné à une amende indéfiniment par la même caméra!
								end
							end
						end
					end
							...
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if alertPolice == true then
										if SpeedKM > alertSpeed then
											local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))
											TriggerServerEvent('esx_phone:send', 'police', " Quelqu'un est passée devant le radar, a cette vitesse " .. alertSpeed.. ' KMH', true, {x =x, y =y, z =z})
										end
									end
									-- ALERT POLICE (END)								
									...
									TriggerEvent("pNotify:SendNotification", {text = "Tu a ete attrapé par le radar dans la zone 80 ! ta vitesse a été de: " .. math.floor(SpeedKM) .. " KM/H", type = "error", timeout = 5000, layout = "centerLeft"})
