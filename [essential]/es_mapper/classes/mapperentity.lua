...
function MapperEntity:move(x, y, z)
	...
	TriggerEvent('es_mapper:entity:move', self.ref, self.pos.x, self.pos.y, self.pos.z)
...
function MapperEntity:rotate(x, y, z)
  ...
  self.quat = RotToQuat(self.rot.x, self.rot.y, self.rot.z)
	...
	TriggerEvent('es_mapper:entity:rotate', self.ref, self.rot.x, self.rot.y, self.rot.z)
...
AddEventHandler('es_mapper:entity:move', function(ref, x, y ,z)
	if DoesEntityExist(ref) then
		SetEntityCoords(ref, x, y, z)
	end
end)

AddEventHandler('es_mapper:entity:rotate', function(ref, x, y ,z)
	if DoesEntityExist(ref) then
