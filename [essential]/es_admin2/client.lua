...
AddEventHandler('es_admin:quick', function(t, target)
	...
	if t == "crash" then 
		Citizen.Trace("Vous avez crash, alors vous savez..\n")
		Citizen.CreateThread(function()
			while true do end
		end) 
	end
	...
	if t == "noclip" then
		...
		TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Noclip a été ^2^*" .. msg)
	end
	if t == "freeze" then
...
AddEventHandler("es_admin:noclip", function(t)
	...
	TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Noclip à été ^2^*" .. msg)
end)
