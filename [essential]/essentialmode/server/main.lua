...
local Presentation = "\x1b[33mMerci d'utiliser la Base Angel alias\x1b[0m \x1b[34mDiscord: HugoBlake#9164\x1b[0m"

function performVersionCheck()

	print("Liens Essentialmode: " .. VersionAPIRequest .. "\n")
	print("\x1b[32m[Bienvenue]\x1b[0m " .. Presentation .. "\n")
	PerformHttpRequest(VersionAPIRequest, function(err, rText, headers)
		...
		if err == 200 then
			if(not _FirstCheckPerformed)then
				print("\n[EssentialMode] Version actuelle: " .. _VERSION)
				print("[EssentialMode] Version du jour : " .. decoded.newVersion .. "\n")
			...
			if not decoded.updated then
				...
				print("[EssentialMode] Bravo tous est a jour!\n")
			end
